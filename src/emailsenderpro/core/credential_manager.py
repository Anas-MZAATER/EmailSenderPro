...
class CredentialManager:
    ...
    def delete_password(self, username: str) -> bool:
        ...
        if self._use_keyring:
            try:
                ...
                pass
        self._remove_from_env(username)
        return success
    ...
    def _save_to_env(self, username: str, password: str) -> bool:
        ...
        prefix = f"SMTP_PASSWORD_{username}="
        lines = [l for l in lines if not l.startswith(prefix)]
        lines.append(f"{prefix}{password}")
        with open(ENV_FILE, "w", encoding="utf-8") as f:
            f.write("\n".join(lines) + "\n")
        logger.info(f"Password saved to .env for {username}")
        return True

    def _get_from_env(self, username: str) -> Optional[str]:
        if ENV_FILE.exists():
            load_dotenv(ENV_FILE)
            return os.getenv(f"SMTP_PASSWORD_{username}")
        return None

    def _remove_from_env(self, username: str) -> None:
